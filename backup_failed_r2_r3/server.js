...
async function startDrawingPhase(roomCode, forcedRound = null) {
  ...
  const players = await getPlayersInRoom(roomCode);
  const round = forcedRound || (room ? room.round : 1) || 1;
  const totalRounds = 3;
  console.log(`[ROUND LOG] startDrawingPhase: Round ${round}/${totalRounds} in ${roomCode}`);
  
  // Points: R1=100, R2=200, R3=200
  const pointsBase = (round >= 2) ? 200 : 100;
...
async function startObservePhase(roomCode, stage = 1) {
  clearRoomTimers(roomCode);
  const room = await getRoomData(roomCode);
  const players = await getPlayersInRoom(roomCode);
  ...
  // Round 3 is now the Layered Copying round (5+ players)
  const isLayered = (round === 3 && playerCount >= 5);

  if (stage === 1) {
    // Assign copy targets (P B copies P A)
    ...
    // Also prepare for Stage 2 if layered
    if (isLayered) {
      payload.copyAssignments2 = {};
      playerIds.forEach((id, idx) => {
        // Player C copies Player B (who is copying Player A)
        // P(idx+2) copies P(idx+1)
        payload.copyAssignments2[playerIds[(idx + 2) % playerIds.length]] = playerIds[(idx + 1) % playerIds.length];
      });
    }
  }

  if (supabase) {
    await supabase.from('rooms').update({ phase: isLayered ? `observe${stage}` : 'observe', payload }).eq('code', roomCode);
  } else {
    mockRooms[roomCode].phase = isLayered ? `observe${stage}` : 'observe';
    mockRooms[roomCode].payload = payload;
  }

  players.forEach(p => {
    const targetId = (stage === 1) ? payload.copyAssignments[p.id] : payload.copyAssignments2[p.id];
    let dataUrl = null;
    if (stage === 1) {
      dataUrl = payload.drawings[targetId]?.dataUrl;
    } else {
      // Stage 2: Observe the COPY made by targeting stage 1 maker
      dataUrl = payload.copies[targetId]?.dataUrl;
    }
  ...
  const t = setTimeout(() => startCopyPhase(roomCode, stage), 10 * 1000);
  gameTimers[roomCode] = [t];
}

async function startCopyPhase(roomCode, stage = 1) {
  clearRoomTimers(roomCode);
  const room = await getRoomData(roomCode);
  const round = room.round || 1;
  const players = await getPlayersInRoom(roomCode);
  const isLayered = (round === 3 && players.length >= 5);

  const duration = Math.max(50, 80 - (round * 10));

  if (supabase) {
    await supabase.from('rooms').update({ phase: isLayered ? `copying${stage}` : 'copying' }).eq('code', roomCode);
  } else {
    mockRooms[roomCode].phase = isLayered ? `copying${stage}` : 'copying';
  }
  ...
  const nextAction = (isLayered && stage === 1) ? () => startObservePhase(roomCode, 2) : () => startVotingPhase(roomCode);
  const t = setTimeout(nextAction, duration * 1000);
  gameTimers[roomCode] = [t];
}
...
async function startVotingPhase(roomCode) {
  ...
  const gallery = players.map(originalOwner => {
    ...
    const copy1 = players.find(p => payload.copies[p.id] && payload.copies[p.id].originalOwnerId === originalOwner.id);
    const copy2 = isLayered ? players.find(p => payload.copies2 && payload.copies2[p.id] && payload.copies2[p.id].targetId === copy1?.id) : null;
    ...
    if (copy1) {
      entries.push({
        playerId: copy1.id,
        username: copy1.username,
        dataUrl: payload.copies[copy1.id].dataUrl,
        isOriginal: false,
        type: 'copy1'
      });
    }
    ...
    if (copy2) {
      entries.push({
        playerId: copy2.id,
        username: copy2.username,
        dataUrl: payload.copies2[copy2.id].dataUrl,
        isOriginal: false,
        type: 'copy2'
      });
    }
...
async function nextPhaseAfterVoting(roomCode) {
  ...
  if (currentRound >= totalRounds) {
    ...
    io.to(roomCode).emit('phase-change', { phase: 'ended', players: finalPlayers });
     ...
     if (supabase) {
       await supabase.from('rooms').update({ phase: 'results', round: currentRound }).eq('code', roomCode);
     } else {
       mockRooms[roomCode].phase = 'results';
       mockRooms[roomCode].round = currentRound;
     }
     ...
     const t = setTimeout(async () => {
        ...
        if (nextRound === 2) {
          console.log(`[ROUND LOG] R2 Transition for ${roomCode}`);
          io.to(roomCode).emit('phase-change', { 
            phase: 'round-start', 
            round: 2, 
            duration: 5,
            info: "round 2: double points!"
          });
          const t2 = setTimeout(() => {
            startDrawingPhase(roomCode, 2);
          }, 5 * 1000);
          gameTimers[roomCode] = [t2];
        } else if (nextRound === 3) {
          console.log(`[ROUND LOG] R3 Transition (Final Layered) for ${roomCode}`);
          if (supabase) {
          ...
          io.to(roomCode).emit('phase-change', { 
            ...
            round: 3, 
            duration: 6,
            info: "final round: copy-of-copy! (5+ players)" 
          });
          const t3 = setTimeout(() => {
            startDrawingPhase(roomCode, 3);
          }, 6000);
          gameTimers[roomCode] = [t3];
        } else {
...
io.on('connection', (socket) => {
  ...
  socket.on('submit-copy', async ({ roomCode, playerId, dataUrl }) => {
     ...
     if (phase !== 'copying' && phase !== 'copying1' && phase !== 'copying2') return;
     
     const payload = room.payload || {};
     const stage = (phase === 'copying2') ? 2 : 1;

     if (stage === 1) {
       payload.copies[playerId] = { dataUrl, originalOwnerId: payload.copyAssignments ? payload.copyAssignments[playerId] : null };
     } else {
       if (!payload.copies2) payload.copies2 = {};
       payload.copies2[playerId] = { dataUrl, targetId: payload.copyAssignments2 ? payload.copyAssignments2[playerId] : null };
     }
     ...
     const activePlayers = await getActivePlayersInRoom(roomCode);
     const currentCopies = (stage === 1) ? payload.copies : payload.copies2;
     
     if (activePlayers.every(p => currentCopies[p.id])) {
        if (stage === 1 && room.round === 3 && activePlayers.length >= 5) {
          startObservePhase(roomCode, 2);
        } else {
  ...
  socket.on('submit-vote', async ({ roomCode, playerId, choices }) => {
    ...
    if (isParticipant) {
      console.log(`[VOTE LOG] ${playerId} is a participant in this set, ignoring vote.`);
      return;
    }
